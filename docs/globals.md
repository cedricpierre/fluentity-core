[**Fluentity Core Documentation**](README.md)

***

# Fluentity Core Documentation

## Classes

- [HttpClient](classes/HttpClient.md)
- [Model](classes/Model.md)
- [RelationBuilder](classes/RelationBuilder.md)

## Interfaces

- [Attributes](interfaces/Attributes.md)
- [HttpRequest](interfaces/HttpRequest.md)
- [RequestOptions](interfaces/RequestOptions.md)

## Type Aliases

- [Constructor](type-aliases/Constructor.md)
- [HttpResponse](type-aliases/HttpResponse.md)
- [MethodType](type-aliases/MethodType.md)
- [PropertyDecoratorType](type-aliases/PropertyDecoratorType.md)
- [Relation](type-aliases/Relation.md)

## Variables

- [BelongsTo](variables/BelongsTo.md)
- [BelongsToMany](variables/BelongsToMany.md)
- [Methods](variables/Methods.md)

## Functions

- [Cast](functions/Cast.md)
- [fetchRequestHandler](functions/fetchRequestHandler.md)
- [HasMany](functions/HasMany.md)
- [HasOne](functions/HasOne.md)

## References

### Fluentity

Renames and re-exports [HttpClient](classes/HttpClient.md)
