...
describe('GraphqlAdapter', () => {
  ...
  it('should send a POST request with a body graphql query', async () => {
    const queryBuilder = new QueryBuilder({ 
      model: User as typeof Model,
    });
    
    await httpClient.call(queryBuilder);
    expect(httpClient.request.body.query.replace(/\s+/g, ' ').trim()).toEqual(`
      query GetUsers {
        users {
          id
        }
    ...
    `.replace(/\s+/g, ' ').trim());
    expect(httpClient.request.body.variables).toEqual({});
    expect(httpClient.request.body.operationName).toEqual('GetUsers');
  });
});
